...
int main() {
	...
	float r1CheckValues[5] = {0.0660100058, 0.451010048, 0.478009999, 0.704010010, 0.772009969};	// Hard coded values from routine1
...
void routine2_vec(float alpha, float beta) {
	...
	for (i = 0; i < N; i+=8)
	{
		...
		__m256 subResult = _mm256_sub_ps(wContain, betaContain);	// This does not need to be recalculated
		...
		for (j = 0; j < N; j+=8)									// every j loop
		{
			...
			__m256 multResult = _mm256_mul_ps(_mm256_mul_ps(alphaContain, arrAContain), xContain);	// Saves a variable

			__m256 finalResult = _mm256_add_ps(subResult, multResult);
